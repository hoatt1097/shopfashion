...
<div style="position:absolute; left:270px;">
 <h1>Đời sống gia đình</h1>
 <div id='noidung'> 
       <div class="tech_store_index_col_1 colection">
        <div class="row">
          <div class="col-xs-12 index_col_products">
 	<?php
	 $dt->select('SELECT * from bangtin where idnhom = 4 order by ngaydang desc limit 1');
	 if ($r=$dt->fetch()){
		 $id=$r['idtin'];
		 $idn=$r['idnhom'];
		 $tieude=$r['tieude'];
	...
	<?php
	 ...
	 $dt->select("SELECT * from bangtin where idnhom = 4 and ngaydang != '$newest' order by ngaydang desc limit 10");
	?>
        <div class="row">
          <div class="col-xs-12 index_col_products">
    <?php
