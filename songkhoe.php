...
<div style="position:absolute; left:270px;">
 ...
 <div id='noidung'> 
 	...
 	<?php
	 $dt->select('SELECT * from bangtin where idnhom = 2 order by ngaydang desc limit 1');
	 if ($r=$dt->fetch()){
		 $id=$r['idtin'];
		 $idn=$r['idnhom'];
		 $tieude=$r['tieude'];
	...
	<?php
	 ...
	 $dt->select("SELECT * from bangtin where idnhom = 2 and ngaydang != '$newest' order by ngaydang desc limit 10");
	?>
        <div class="row">
          <div class="col-xs-12 index_col_products">
    <?php
